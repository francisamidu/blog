...
1. `[slug] page returning 404 when trying to access an article`. The issue here was that the individual page for an article was not generated properly. I changed the code for generating a new slug page to look like this:
```jsx
...
```
2. `SyntaxError: unknown: Unterminated JSX contents. (23:16)`. This was an issue of unterminated element. I found that I has a `<br>` tag not closed. Solved it by terminating it like so:`<br />`;
